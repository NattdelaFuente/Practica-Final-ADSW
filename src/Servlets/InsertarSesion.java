...
public class InsertarSesion extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ajax) { // Handle ajax (JSON or XML) response.
			...
			if (dao.esPosibleSesion(idSala, milisInicio, milisFinal)) {
				if (dao.insertarSesion(idPelicula, idSala, fechaInicio, horaInicio, milisInicio, fechaFinal, horaFinal,
						milisFinal)) {

					System.out.println("Creada la sesion ");
					data.put("success", "Sesión guardada con éxito");
					String json = new Gson().toJson(data);
					response.getWriter().write(json);
				}

				else {
					System.out.println("Fallo al crear sesion");
					data.put("error", "La sesion no ha podido ser guardada");
					String json = new Gson().toJson(data);
					response.getWriter().write(json);
