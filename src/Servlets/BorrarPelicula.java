...
public class BorrarPelicula extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ajax) { // Handle ajax (JSON or XML) response.
			...
			if (dao.borrarPelicula(id)) {

				System.out.println("borrada la pelicula " + id);
				data.put("success", "película borrada con éxito");
				String json = new Gson().toJson(data);
				response.getWriter().write(json);
			}

			else {
				System.out.println("Fallo al borrar pelicula");
				data.put("error", "la película no ha podido ser borrada");
				String json = new Gson().toJson(data);
				response.getWriter().write(json);
